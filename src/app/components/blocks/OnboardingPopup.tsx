...
const stepsContent: TStepContent[] = [
  {
    ...
    description:
      "Instantly trade over 2,500 tokens, connect with hundreds of dApps, and explore the Smart Energy Chain ecosystem through SEP Wallet",
    image: OnboardingFirstImage,
  },
  {
    title: "Easy access",
    description:
      "Connect to dApps seamlessly with SEP Wallet and access all your Smart Energy Chain assets in one place",
    image: OnboardingSecondImage,
  },
  {
    title: "Always near",
    description:
      "Pin the SEP Wallet extension on your browser toolbar for fast access to your tokens and to navigate the Smart Energy Chain ecosystem efficiently",
    image: OnboardingThirdImage,
  },
];

const OnboardingPopup: FC<OnboardingPopupProps> = (props) => {
  const visible = useAtomValue(onboardingPopupAtom);

  if (props.onClose && props.onContinue) {
    return <OnBoardingContent onClose={props.onClose} onContinue={props.onContinue} />;
  }
...
const OnBoardingContent: FC<OnBoardingContentProps> = ({ onClose, onContinue }) => {
      ...
      <ScrollAreaContainer className="w-full h-full flex flex-col relative max-w-7xl">
        <section className="mb-6 flex flex-col items-center">
          ...
          <h3 className="mb-6 text-[1.75rem] font-bold text-center">
            ...
            <br />
            Your Gateway to Smart Energy Chain!
          </h3>
          <Stepper step={step} />
        </section>
...
const StepLine: FC<{ active: boolean }> = ({ active }) => (
  <span
    className={classNames(
      ...
      "relative after:absolute after:h-full after:rounded-full after:bg-[#4CAF50]",
