...
const ADDITIONAL_PLATFORM_COINS = new Map([
  [800001, "octaspace"],
  [19516, "smart-energy-pay"],
  [19515, "smart-energy-pay"]
]);
...
// Mock CoinGecko API responses for critical endpoints
const originalCoinGeckoGet = coinGeckoApi.get;
coinGeckoApi.get = async function(url: string, config?: any) {
  try {
    // First try the actual API call
    return await originalCoinGeckoGet(url, config);
  } catch (error) {
    console.warn(`CoinGecko API call to ${url} failed, using mock data`, error);
    
    // Mock responses for critical endpoints
    if (url === "/asset_platforms") {
      return {
        data: [
          {
            id: "ethereum",
          ...
          {
            id: "smart-energy-chain",
            chain_identifier: 19516,
            name: "Smart Energy Chain",
            shortname: "SEP",
            native_coin_id: "smart-energy-pay"
          },
          {
            id: "smart-energy-chain-testnet",
            chain_identifier: 19515,
        ...
        data: [
          {
            ...
            platforms: {
              "smart-energy-chain": "0x0000000000000000000000000000000000000000",
              "smart-energy-chain-testnet": "0x0000000000000000000000000000000000000000"
            }
          }
        ]
      };
...
} as typeof originalCoinGeckoGet;
